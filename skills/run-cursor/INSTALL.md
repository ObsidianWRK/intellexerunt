# Installing `run-cursor`

Execute a UI/frontend task via Cursor models (Composer 2) natively through cursor-bridge MCP.
...
```bash
cp -r skills/run-cursor .claude/skills/run-cursor
```
...
1. Zip the skill folder: `cd skills && zip -r run-cursor.zip run-cursor/`
...
```bash
cp -r skills/run-cursor .cursor/skills/run-cursor
```
...
```bash
cp -r skills/run-cursor .agents/skills/run-cursor
```
...
```bash
cp -r skills/run-cursor .github/skills/run-cursor
```
...
```bash
cp -r skills/run-cursor .gemini/skills/run-cursor
```
