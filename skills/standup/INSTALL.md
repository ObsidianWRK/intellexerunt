...
Use when the user asks for a standup, status summary, or progress report. Triggers on "what did we d
...
1. Zip the skill folder: `cd skills && zip -r standup.zip standup/`
...
```bash
cp -r skills/standup .cursor/skills/standup
```
...
```bash
cp -r skills/standup .agents/skills/standup
```
...
```bash
cp -r skills/standup .github/skills/standup
```
...
```bash
cp -r skills/standup .gemini/skills/standup
```
