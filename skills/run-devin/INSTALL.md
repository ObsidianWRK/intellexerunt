# Installing `run-devin`

Use when the user asks to trigger Devin review on a PR. Triggers on "ask devin", "have devin look at
...
```bash
cp -r skills/run-devin .claude/skills/run-devin
```
...
1. Zip the skill folder: `cd skills && zip -r run-devin.zip run-devin/`
...
```bash
cp -r skills/run-devin .cursor/skills/run-devin
```
...
```bash
cp -r skills/run-devin .agents/skills/run-devin
```
...
```bash
cp -r skills/run-devin .github/skills/run-devin
```
...
```bash
cp -r skills/run-devin .gemini/skills/run-devin
```
