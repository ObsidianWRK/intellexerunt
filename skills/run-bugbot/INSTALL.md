# Installing `run-bugbot`

Use when the user asks to trigger BugBot review on a PR. Triggers on "run bugbot", "bug finder", or 
...
```bash
cp -r skills/run-bugbot .claude/skills/run-bugbot
```
...
1. Zip the skill folder: `cd skills && zip -r run-bugbot.zip run-bugbot/`
...
```bash
cp -r skills/run-bugbot .cursor/skills/run-bugbot
```
...
```bash
cp -r skills/run-bugbot .agents/skills/run-bugbot
```
...
```bash
cp -r skills/run-bugbot .github/skills/run-bugbot
```
...
```bash
cp -r skills/run-bugbot .gemini/skills/run-bugbot
```
