# Installing `setup-browser-cookies`

Use when the user asks to setup browser cookies, import sessions, or authenticate the headless brows
...
```bash
cp -r skills/setup-browser-cookies .claude/skills/setup-browser-cookies
```
...
1. Zip the skill folder: `cd skills && zip -r setup-browser-cookies.zip setup-browser-cookies/`
...
```bash
cp -r skills/setup-browser-cookies .cursor/skills/setup-browser-cookies
```
...
```bash
cp -r skills/setup-browser-cookies .agents/skills/setup-browser-cookies
```
...
```bash
cp -r skills/setup-browser-cookies .github/skills/setup-browser-cookies
```
...
```bash
cp -r skills/setup-browser-cookies .gemini/skills/setup-browser-cookies
```
