# Installing `document-release`

Use when the user asks to update docs for a release, write a changelog, or update README/CONTRIBUTIN
...
```bash
cp -r skills/document-release .claude/skills/document-release
```
...
1. Zip the skill folder: `cd skills && zip -r document-release.zip document-release/`
...
```bash
cp -r skills/document-release .cursor/skills/document-release
```
...
```bash
cp -r skills/document-release .agents/skills/document-release
```
...
```bash
cp -r skills/document-release .github/skills/document-release
```
...
```bash
cp -r skills/document-release .gemini/skills/document-release
```
