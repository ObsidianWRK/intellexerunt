# Installing `run-codex`

Execute a task via Codex CLI (GPT 5.4) in an isolated worktree.
...
```bash
cp -r skills/run-codex .claude/skills/run-codex
```
...
1. Zip the skill folder: `cd skills && zip -r run-codex.zip run-codex/`
...
```bash
cp -r skills/run-codex .cursor/skills/run-codex
```
...
```bash
cp -r skills/run-codex .agents/skills/run-codex
```
...
```bash
cp -r skills/run-codex .github/skills/run-codex
```
...
```bash
cp -r skills/run-codex .gemini/skills/run-codex
```
